			...
			switch ( $item_name ) {
				case 'php_min_required_version':
					if ( ! empty( $item_details['value'] ) && version_compare( phpversion(), $item_details['value'], '<' ) ) {
						...
						$this->messages[] = sprintf( __( 'The minimum PHP version required is %s', 'restricted-site-access' ), $item_details['value'] );
					}
					break;
				...
				case 'php_max_required_version':
					if ( ! empty( $item_details['value'] ) && version_compare( phpversion(), $item_details['value'], '>' ) ) {
						// translators: %s: PHP version
						$this->messages[] = sprintf( __( 'The maximum PHP version supported is %s', 'restricted-site-access' ), $item_details['value'] );
					}
					break;
				...
				case 'wp_min_required_version':
					if ( ! empty( $item_details['value'] ) && version_compare( $this->wp_get_wp_version(), $item_details['value'], '<' ) ) {
						...
						$this->messages[] = sprintf( __( 'The minimum WordPress version required is %s', 'restricted-site-access' ), $item_details['value'] );
					}
					break;
				...
				case 'wp_max_required_version':
					if ( ! empty( $item_details['value'] ) && version_compare( $this->wp_get_wp_version(), $item_details['value'], '>' ) ) {
						// translators: %s: WordPress version
						$this->messages[] = sprintf( __( 'The maximum WordPress version supported is %s', 'restricted-site-access' ), $item_details['value'] );
					}
					break;
			...
			<p>
				<strong>
					<?php
						...
						printf( esc_html__( '%s error:', 'restricted-site-access' ), esc_html( $this->checklist['plugin_name']['value'] ) );
					?>
				</strong>
			</p>
